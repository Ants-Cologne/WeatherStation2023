...
# Public Domain
# Modified for use with ANTSTATION by Ants.Cologne 2022 (I only changed __main__)
...
if __name__== "__main__":
   ...
   try:
      conn = mariadb.connect(
         user="root",			# We're using root as this script only runs on localhost
         password="password",		# I'm using a different pwd, of course ;)
         host="127.0.0.1",		# localhost, no access from outside
         port=3306,			# same port as from outside
         database="antstation"	# Ants.Cologne: in this db we're storing our sensor data!
      )
   ...
   while True:
      try:
         for s in S:
            if s[0] >= 100:
            ...
            else:       # This is only relevant for ANTSTATION
               d = s[1].read()
               print("{:.3f} {:2d} {} {:3.1f} {:3.1f}".
               ...
               if d[2] == 0:         # We only write to db if sensor data was OK
                  try:
                     cur.execute(
                        "INSERT INTO raw_data (temp,hygro,sensor_id) VALUES (?,?,?)",
                        (d[3],d[4],d[1]))
                     print("Data written to database. Sensor: " + str(d[1]))
                  except mariadb.Error as e:
                     print(f"error: {e}")
